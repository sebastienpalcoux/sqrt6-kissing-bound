...
lemma upperProfile3_integral :
  ...
  calc
    ...
    _ = (2 : ℝ) / 3 - 3 * s3 / 8 := by
      have hconst : IntervalIntegrable (fun _ : ℝ => (1 : ℝ)) volume a 1 := by
        apply Continuous.intervalIntegrable
        fun_prop
      have hpow : IntervalIntegrable (fun t : ℝ => t ^ 2) volume a 1 := by
        apply Continuous.intervalIntegrable
        fun_prop
...
lemma code_card_le_sqrt6_cube
  ...
  have hpow3 : (Real.sqrt 6) ^ 3 = 6 * Real.sqrt 6 := by
    calc
