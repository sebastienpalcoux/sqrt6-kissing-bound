...
/-- A reflection carrying the first standard unit vector to a unit vector x. -/
def axisTo (k : ℕ) (x : EuclideanSpace ℝ (Fin (k + 2))) :
    EuclideanSpace ℝ (Fin (k + 2)) ≃ₗᵢ[ℝ] EuclideanSpace ℝ (Fin (k + 2)) :=
...
lemma measurableSet_centeredBicone (k : ℕ)
    (x : EuclideanSpace ℝ (Fin (k + 2))) : MeasurableSet (centeredBicone k x) :=
  (measurableSet_standardBicone k).preimage (axisTo k x).symm.continuous.measurable

lemma volumeReal_centeredBicone (k : ℕ) (x : EuclideanSpace ℝ (Fin (k + 2))) :
    (volume : Measure (EuclideanSpace ℝ (Fin (k + 2)))).real (centeredBicone k x) =
      (1 / ((k + 2) * 2 ^ (k + 1))) * unitBallVolumeReal (k + 1) := by
  rw [centeredBicone, Measure.real,
    (axisTo k x).symm.measurePreserving.measure_preimage
      (measurableSet_standardBicone k).nullMeasurableSet, ← Measure.real]
