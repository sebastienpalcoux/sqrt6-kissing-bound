# Guide to the complete geometric certificate

This guide explains the Lean proof whose exact sources passed the complete build and axiom audit. `CERTIFICATION_STATUS.md` identifies the verified staging run; the permanent certificate artifact records its direct-source verification run and commit.

## Statement and normalization

A normalized kissing configuration is a finite set X of vectors in real Euclidean n-space satisfying norm(x) = 1 and dist(x,y) >= 1 for distinct members. The distance condition is proved equivalent to inner(x,y) <= 1/2. Scaling the centers of equal unit balls tangent to a central unit ball by 1/2 gives this normalization.

The target theorem bounds |X| by (sqrt(6))^n for every positive integer n. The definition of the kissing number is accompanied by proofs that its value is realized and that it is at least every realizable configuration cardinality. The final bound is therefore about a genuine attained maximum, not an assumed function.

## Disjoint cones and exact volumes

For a unit vector x, consider the open cone
...
Both pieces lie in the open unit ball and the strict cone. Fubini's theorem and volume-preserving coordinates give its exact n-dimensional volume
...
For n >= 6, the unit-ball volume recurrence proves V_n <= V_(n-1), and a two-step induction proves n*2^(n-1) <= (sqrt(6))^n. The cases n = 2 and n = 5 follow from exact low-dimensional unit-ball volumes and the integral nature of |X|.
...
The end-to-end geometric proof uses ambient-volume packing and the explicit bodies above. It is an alternative proof of the manuscript's principal results, not a claim to have translated every sentence of its spherical-cap proof. The final theorem does not assume a cap-area formula, a packing inequality, or exceptional-dimensional estimates.

The original manuscript is preserved unchanged. Its historical disclosure about the earlier conditional certificate should not be mistaken for the status of this later, separately audited certificate.
